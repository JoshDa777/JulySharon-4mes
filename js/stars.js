/* =====================================================================
   🌟 FONDO DE ESTRELLAS + CONFETI (optimizado para rendimiento)
   ===================================================================== */
...
(function () {
  ...
  function redimensionar() {
    // Limitar DPR a 1.5 para reducir carga
  ...
  function crearEstrellas() {
    // Reducir cantidad de estrellas para rendimiento
    const cantidad = Math.min(120, Math.floor((ancho * alto) / 12000));
    estrellas = [];
    for (let i = 0; i < cantidad; i++) {
      estrellas.push({
        ...
        rosa: Math.random() < 0.18,
      });
    }
  }

  function crearCometas() {
    // Cometas ocasionales (menos frecuentes)
    if (Math.random() < 0.002 && cometas.length < 1) {
      cometas.push({
        x: Math.random() * ancho,
        y: -30,
  ...
  function lanzarConfeti() {
    const dpr = Math.min(window.devicePixelRatio || 1, 1.5);
    confetiCanvas.width = window.innerWidth * dpr;
    confetiCanvas.height = window.innerHeight * dpr;
    cctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    const colores = ["#ff69b4", "#ffb6d9", "#d63384", "#ffd700", "#ffffff", "#ff85c2"];
    piezas = [];
    const cantidad = 100; // reducido para rendimiento
    for (let i = 0; i < cantidad; i++) {
      piezas.push({
  ...
  function animarConfeti() {
    ...
    for (let i = piezas.length - 1; i >= 0; i--) {
      ...
      if (p.y < window.innerHeight + 30 && p.vida > 0) activos = true;
    }
