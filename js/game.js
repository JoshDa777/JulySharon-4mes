/* =====================================================================
   ...
   Optimizado para rendimiento (sin lag ni congelación).
   ===================================================================== */
...
(function () {
  ...
  function ajustarCanvas() {
    // Limitar DPR a 1.5 para reducir la carga de píxeles (evita lag)
  ...
  /* ---------- PARTÍCULAS (limitadas para rendimiento) ---------- */
  function spawnParticulas(x, y, color, n, spd) {
    const max = 40; // límite total de partículas para evitar lag
    for (let i = 0; i < (n || 8); i++) {
      if (particulas.length >= max) break;
  ...
  function danoAlJugador() {
    ...
    if (estado.vidas <= 0) {
      ...
      const fraseMuerte = aleatorio(CONFIG.frasesMuerte);
      if (fraseMuerte) toast(fraseMuerte, 3200);
  ...
  function comprobarProgreso() {
    for (let i = 0; i < estaciones.length; i++) {
      ...
      if (!st.alcanzada && Math.hypot(jugador.x - st.x, (jugador.y - 4) - st.y) < 95) {
        ...
        const fraseEstacion = aleatorio(CONFIG.frasesEstacion);
        toast((fraseEstacion ? fraseEstacion + "  💖  " : "") + "(" + estado.estaciones + "/5)", 3600);
      }
    }
  ...
  function actualizar(dt) {
    ...
    if (proximaFrase <= 0) {
      ...
      const frase = aleatorio(CONFIG.frasesJuego);
      if (frase) toast(frase, 2400);
    }
    ...
    // Estela de la papita al correr (limitada)
    if ((teclas.izq || teclas.der) && jugador.enSuelo && Math.random() < 0.15 && particulas.length < 40) {
      particulas.push({
  ...
  /* ---------- DIBUJAR (optimizado: sin gradientes por frame) ---------- */
  function dibujarParallax() {
    const px = camara.x * 0.18;

    // Nebulosa (color sólido, sin gradiente por frame)
    ctx.fillStyle = "rgba(124,58,237,0.10)";
    ctx.beginPath();
    ctx.arc(VW * 0.25 - px, VH * 0.22, VH * 0.4, 0, Math.PI * 2);
    ctx.fill();
  ...
  function dibujarPlataforma(pl) {
    ...
    // Cuerpo rocoso espacial (color sólido, sin gradiente)
  ...
  function dibujarEstacion(st, i) {
    ...
    // Halo (color sólido, sin gradiente)
  ...
  function dibujarFondoPolvo() {
    // Pequeño polvo estelar con parallax (reducido a 30 para rendimiento)
  ...
  function gameOver() {
    estado.jugando = false;
    estado.pausado = false;
    $("muerte-texto").textContent = aleatorio(CONFIG.frasesMuerte) || "¡Pero el universo es inmenso y el mapa cambia! Inténtalo otra vez, mi amor 🌸";
